/**
 * Vérification rapide (IDs uniquement) et sync par IDs (SyncOdoo autonome)
 */
...
async function authenticate(): Promise<number | null> {
  ...
  if (!contentType || !contentType.includes("application/json")) throw new Error("Réponse invalide");
...
async function odooSearch(model: string, domain: any[]): Promise<number[]> {
  ...
  if (!uid) throw new Error("Échec auth");
...
export async function checkQuotesFast(): Promise<number[]> {
  try {
    ...
    const [modifiedIds, unconfirmedIds] = await Promise.all([
      odooSearch("sale.order", [["write_date", ">", toOdooDatetime(checkDate)]]),
      odooSearch("sale.order", [["state", "in", ["draft", "sent"]]]),
    ]);
    const allIds = Array.from(new Set([...modifiedIds, ...unconfirmedIds]));
    await updateLastFastCheckDate("quotes");
    if (allIds.length > 0) console.log(`[Fast Check] ${allIds.length} devis à synchroniser`);
    return allIds;
  } catch (error) {
...
export async function checkInvoicesFast(): Promise<number[]> {
  try {
    ...
    if (ids.length > 0) console.log(`[Fast Check] ${ids.length} factures à synchroniser`);
    return ids;
  } catch (error) {
...
export async function checkPartnersFast(): Promise<number[]> {
  try {
    ...
    if (ids.length > 0) console.log(`[Fast Check] ${ids.length} clients à synchroniser`);
    return ids;
  } catch (error) {
...
export async function checkProductsFast(): Promise<number[]> {
  try {
    ...
    if (ids.length > 0) console.log(`[Fast Check] ${ids.length} produits à synchroniser`);
    return ids;
  } catch (error) {
...
export async function syncQuotesByIds(ids: number[]): Promise<void> {
  ...
  try {
    ...
    await pgUpsert("odoo_quotes", quotes.map((q: any) => ({
      odoo_id: q.id, name: q.name, odoo_partner_id: Array.isArray(q.partner_id) ? q.partner_id[0] : q.partner_id, partner_name: Array.isArray(q.partner_id) ? q.partner_id[1] : null,
      date_order: q.date_order ? new Date(q.date_order) : null, commitment_date: q.commitment_date ? new Date(q.commitment_date) : null,
      amount_total: q.amount_total || 0, amount_untaxed: q.amount_untaxed || 0, state: q.state || null,
      odoo_user_id: Array.isArray(q.user_id) ? q.user_id[0] : null, user_name: Array.isArray(q.user_id) ? q.user_id[1] : null,
      odoo_write_date: q.write_date ? new Date(q.write_date) : null, odoo_data: q, synced_at: new Date().toISOString(),
    })));
    console.log(`[Fast Sync] ${quotes.length} devis synchronisés`);
  } catch (error) {
...
export async function syncInvoicesByIds(ids: number[]): Promise<void> {
  ...
  try {
    ...
    await pgUpsert("odoo_invoices", invoices.map((inv: any) => ({
      odoo_id: inv.id, name: inv.name, odoo_partner_id: Array.isArray(inv.partner_id) ? inv.partner_id[0] : inv.partner_id, partner_name: Array.isArray(inv.partner_id) ? inv.partner_id[1] : null,
      invoice_date: inv.invoice_date || null, invoice_date_due: inv.invoice_date_due || null, amount_total: inv.amount_total || 0, amount_untaxed: inv.amount_untaxed || 0,
      amount_residual: inv.amount_residual || 0, state: inv.state || null, payment_state: inv.payment_state || null, move_type: inv.move_type || null, invoice_origin: inv.invoice_origin || null,
      odoo_write_date: inv.write_date ? new Date(inv.write_date) : null, odoo_data: inv, synced_at: new Date().toISOString(),
    })));
    console.log(`[Fast Sync] ${invoices.length} factures synchronisées`);
  } catch (error) {
...
export async function syncPartnersByIds(ids: number[]): Promise<void> {
  ...
  try {
    ...
    await pgUpsert("odoo_partners", partners.map((p: any) => ({
      odoo_id: p.id, name: p.name, parent_id: Array.isArray(p.parent_id) ? p.parent_id[0] : p.parent_id || null, is_company: p.is_company || false,
      email: p.email || null, phone: p.phone || null, mobile: null, street: p.street || null, street2: p.street2 || null, city: p.city || null, zip: p.zip || null,
      country_id: Array.isArray(p.country_id) ? p.country_id[0] : p.country_id || null, country_name: Array.isArray(p.country_id) ? p.country_id[1] : null, image_128: null,
    ...
    console.log(`[Fast Sync] ${partners.length} clients synchronisés`);
  } catch (error) {
...
export async function syncProductsByIds(ids: number[]): Promise<void> {
  ...
  try {
    ...
    await pgUpsert("odoo_products", products.map((p: any) => ({
      ...
      categ_id: Array.isArray(p.categ_id) ? p.categ_id[0] : p.categ_id || null, categ_name: Array.isArray(p.categ_id) ? p.categ_id[1] : null,
      odoo_write_date: p.write_date ? new Date(p.write_date) : null, odoo_data: p, synced_at: new Date().toISOString(),
    })));
    console.log(`[Fast Sync] ${products.length} produits synchronisés`);
  } catch (error) {
...
export async function fastCheck(): Promise<{ quotesToSync: number; invoicesToSync: number; partnersToSync: number; productsToSync: number }> {
  try {
    const [quoteIds, invoiceIds, partnerIds, productIds] = await Promise.all([
      checkQuotesFast(),
      checkInvoicesFast(),
      checkPartnersFast(),
      checkProductsFast(),
    ...
    await Promise.all([
      quoteIds.length > 0 ? syncQuotesByIds(quoteIds) : Promise.resolve(),
      invoiceIds.length > 0 ? syncInvoicesByIds(invoiceIds) : Promise.resolve(),
      partnerIds.length > 0 ? syncPartnersByIds(partnerIds) : Promise.resolve(),
      productIds.length > 0 ? syncProductsByIds(productIds) : Promise.resolve(),
    ]);
    return { quotesToSync: quoteIds.length, invoicesToSync: invoiceIds.length, partnersToSync: partnerIds.length, productsToSync: productIds.length };
  } catch (error) {
